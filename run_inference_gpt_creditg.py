"""
Evaluates and compares creditg predictions from a trained NN
...
import openai
from utils import prepare_data_creditg, train_nn, truncate, NN, ensure_dir
from config import  PROMPT_TEMPLATE_RAW_CREDITG as template_raw
from config import PROMPT_TEMPLATE_GUIDED_CREDITG as template_guided
...
def print_scores(start_idx, end_idx, x_test, y_test, x_test_orig,
    ...
    Returns:
        ...
        the 'predictions/creditg/' directory.
    """
    ...
    for sample_id in range(start_idx, end_idx):
        ...
        file_raw = f"predictions/creditg/gpt-5_raw_creditg{sample_id}.txt"
        ...
        gpt_output = response.choices[0].message.content.strip()
        file_guided = f"predictions/creditg/gpt-5_guided_creditg{sample_id}.txt"
...
if __name__ == "__main__":
    ...
    NPZ_PATH = "processed_data/creditg_data_processed.npz"
    if not os.path.exists(NPZ_PATH):
        print("Preprocessed data not found. Preparing data...")
        prepare_data_creditg()
    else:
    ...
    MODEL_PATH = "trained_nns/creditg_nn.pth"
    ...
    ensure_dir("predictions/creditg/")
