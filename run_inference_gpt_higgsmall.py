"""
Evaluates and compares higgsmall predictions from a trained NN
...
import openai
from utils import prepare_data_higgsmall, train_nn, truncate, NN, ensure_dir
from config import  PROMPT_TEMPLATE_RAW_HIGGSMALL as template_raw
from config import PROMPT_TEMPLATE_GUIDED_HIGGSMALL as template_guided
...
def print_scores(start_idx, end_idx, x_test, y_test, x_test_orig,
    ...
    Returns:
        ...
        the 'predictions/higgsmall/' directory.
    """
    ...
    for sample_id in range(start_idx, end_idx):
        ...
        file_raw = f"predictions/higgsmall/gpt-5_raw_higgsmall{sample_id}.txt"
        ...
        gpt_output = response.choices[0].message.content.strip()
        file_guided = f"predictions/higgsmall/gpt-5_guided_higgsmall{sample_id}.txt"
...
if __name__ == "__main__":
    ...
    NPZ_PATH = "processed_data/higgsmall_data_processed.npz"
    if not os.path.exists(NPZ_PATH):
        print("Preprocessed data not found. Preparing data...")
        prepare_data_higgsmall()
    else:
    ...
    MODEL_PATH = "trained_nns/higgsmall_nn.pth"
    ...
    ensure_dir("predictions/higgsmall/")
